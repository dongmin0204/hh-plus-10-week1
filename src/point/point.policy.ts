export class PointPolicy {
    ...
    static validateChargeAmount(amount: number): void {
        ...
        if (amount > this.MAX_CHARGE_AMOUNT) {
            throw new Error('일회 최대 충전 한도를 초과했습니다. (최대: 100,000 포인트)');
        }
    }
    ...
    static validateUseAmount(amount: number): void {
        ...
        if (amount > this.MAX_USE_AMOUNT) {
            throw new Error('일회 최대 사용 한도를 초과했습니다. (최대: 50,000 포인트)');
        }
    }
     ...
     * 충전 후 잔고 검증
     */
    static validateBalanceAfterCharge(currentBalance: number, chargeAmount: number): void {
        const newBalance = currentBalance + chargeAmount;
        
        if (newBalance > this.MAX_BALANCE) {
            throw new Error('최대 잔고를 초과할 수 없습니다. (최대: 1,000,000 포인트)');
        }
    }
     ...
     * 사용 시 잔고 검증
     */
    static validateBalanceForUse(currentBalance: number, useAmount: number): void {
